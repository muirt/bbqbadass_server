...
class JCmdMove(JsonCommand):
	def __init__(self):	
		JsonCommand.__init__(self, "move", True)
		
	def processCommand(self, value, player):
		
		square = 0

		player_update = {'control': unicodeHelper.getAscii(value), 'value': 'hidden', 'value_type': 'visibility'}
		viewer_update = {'control': unicodeHelper.getAscii(value), 'value': 'visible', 'value_type': 'visibility'}
		viewer_token =  {'control': unicodeHelper.getAscii(value), 'value': player_token[player-1], 'value_type': 'text'}

		player_update_list = []
		viewer_update_list = []

		player_update_list.append(player_update)
		viewer_update_list.append(viewer_update)
		viewer_update_list.append(viewer_token)

		player_message = {
		                    'secret':'badass',
		                    'target':'magic',
		                    'value': player_update_list }

		viewer_message = {
		                    'secret':'badass',
		                    'target':'magic',
		                    'value': viewer_update_list }

		
		result = { "message_type": "multiple", "messages": [player_message, player_message, viewer_message] }
